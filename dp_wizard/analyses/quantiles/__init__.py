...
def make_query(code_gen, identifier, accuracy_name, stats_name):
    return (
        Template("quantiles_query", __file__)
        .fill_values(
...
def make_output(code_gen, column_name, accuracy_name, stats_name):
    return Template(f"quantiles_{code_gen.root_template}_output", __file__).finish()


def make_report_kv(name, confidence, identifier):
    return (
        Template("quantiles_report_kv", __file__)
        .fill_values(
            NAME=name,
        )
        .finish()
...
def make_column_config_block(column_name, lower_bound, upper_bound, bin_count):
    ...
    return (
        Template("quantiles_config", __file__)
        .fill_expressions(
